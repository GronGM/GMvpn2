...
Debug APK со свежими jniLibs (включая Base64 robustness pass) был
установлен на физический TECNO LG8n через `adb install` после удаления
старой debug-сборки с несовпадающей подписью.

Redacted результат ручного импорта реальной подписки на устройстве:

- импорт достиг стадии сохранения: сохранён `1` из `2` профилей;
- второй элемент подписки использует схему `hysteria2://`, которая
  сознательно не поддерживается текущим Xray-путём (см. roadmap P5 в
  `docs/product-direction.md`); он отклонён как per-line warning без
  падения всего импорта — это ожидаемое поведение декодера;
- прежняя форма блокера (`ParseFailed` / `ffi_decode_failed` при
  Base64-теле с `base64DecodeLikely=yes` и `0` сохранённых профилей)
  на этой сборке не воспроизводится.

Raw subscription URL, содержимое тела, URI профилей, hostnames, UUID и
endpoint-данные не фиксировались и не коммитятся.

Статус блокера: импорт-блокер PR #27/#28 на этой сборке снят для
проверенной подписки. Отдельная поддержка `hysteria2://` остаётся
за пределами этого прохода (P5).
