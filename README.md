...
Принят путь MVP/internal `android-v1.0.0` для GitHub release с явно
принятыми ограничениями UDP/IPv6. Это не unrestricted production release
и не публикация в Google Play. Перед финальным tag/release должна пройти
signed GitHub Actions сборка с `versionName` `1.0.0` и `versionCode`
`1000006`, после чего нужны artifact verification и physical smoke.

В release notes для MVP/internal `1.0.0` должны быть зафиксированы:
...
требует signed workflow, artifact verification, physical smoke и
отдельного SHA-specific approval перед tag/release. Тестеры могут скачать текущий
